...
/// Resultado retornado pelo picker
class AddressResult {
  final String formatted;
  final double? lat;
  final double? lng;
  const AddressResult({required this.formatted, this.lat, this.lng});
}

/// Abre o bottom sheet de seleção de endereço.
/// Retorna [AddressResult] com o endereço formatado + coordenadas, ou null se cancelado.
Future<AddressResult?> showAddressPickerSheet(BuildContext context) {
...
class _AddressPickerSheetState extends State<_AddressPickerSheet> {
  ...
  Future<void> _loadCountries() async {
    ...
    setState(() {
      ...
      if (pre != null) _selectedCountry = pre;
    });
  ...
  Future<void> _loadStates(csc.Country country) async {
    setState(() {
      _loadingStates = true; _states = []; _cities = [];
  ...
  Future<void> _geolocate() async {
    ...
    try {
      ...
      if (perm == LocationPermission.deniedForever || perm == LocationPermission.denied) {
        setState(() { _error = 'Permissão de localização negada.'; _locating = false; });
        return;
      }
  ...
  void _submit() {
    if (_selectedCountry == null) {
      setState(() => _error = 'Selecione o país.'); return;
    }
    final street = _streetCtrl.text.trim();
    if (street.isEmpty) {
      setState(() => _error = 'Rua é obrigatória.'); return;
    }
    final parts = [
      street,
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            if (_selectedCountry != null) ...[
              ...
              _loadingStates
                  ...
                  : DropdownButtonFormField<csc.State>(
                      ...
                      decoration: _dropDecoration(_states.isEmpty ? 'Sem estados' : 'Selecione o estado'),
                      isExpanded: true,
                      menuMaxHeight: 300,
                      items: _states.map((s) => DropdownMenuItem(
                        value: s, child: Text(s.name, overflow: TextOverflow.ellipsis),
                      )).toList(),
                      onChanged: _states.isEmpty ? null : (s) async {
                        if (s == null) return;
            ...
            if (_selectedState != null) ...[
              ...
              _loadingCities
                  ...
                  : DropdownButtonFormField<csc.City>(
                      value: _selectedCity,
                      decoration: _dropDecoration(_cities.isEmpty ? 'Sem cidades' : 'Selecione a cidade'),
                      isExpanded: true,
                      menuMaxHeight: 300,
            ...
            _textField('Rua / Avenida', _streetCtrl, hint: 'Rua das Flores', required: true),
            Row(children: [
              Expanded(flex: 2, child: _textField('Número', _numberCtrl, hint: '123', keyboard: TextInputType.number)),
              const SizedBox(width: 10),
              Expanded(flex: 3, child: _textField('Complemento', _complementCtrl, hint: 'Sala 5')),
            ]),
